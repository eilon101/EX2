...
#include <stdlib.h>
#include <string.h>
#define ENGLISHYEAR 1 // Type of year: English
#define HEBREWYEAR 0 // Type of year: Hebrew
...
int main()
{
	...
	worker1 = CreateWorker(ENGLISHYEAR);
	worker2 = CreateWorker(ENGLISHYEAR);
	worker3 = CreateWorker(ENGLISHYEAR);
	PrintWorker(worker1, ENGLISHYEAR);
	WorkerList* List = NULL;
	List = addWorker(List, worker1);
	List = addWorker(List, worker2);
	List = addWorker(List, worker3);
	printList(List, ENGLISHYEAR);
	printf("\nIndex by loop for ID %d: %d\n", 2, indexLoop(List, 2)); // '2' - random number to check the function
	printf("\nIndex by recursion for ID %d: %d\n", 9, indexRecursion(List, 9)); // '9' - random number to check the function
...
Worker* CreateWorker(int Heb_or_Eng)
{
	...
	printf("Enter worker pay:\n");
	scanf("%ld", &NewWorker->pay);
...
WorkerList* addWorker(WorkerList* head, Worker* w)
{
	...
	if (NewElement->data->pay > head->data->pay)
	{
	...
	while ((temp_curr != NULL) && (NewElement->data->pay < temp_curr->data->pay))
	{
		temp_prev = temp_curr; // temp_prev keeps temp_curr's last position
		temp_curr = temp_curr->next;
	} // temp_curr now points to list element with lower pay than NewElement OR points to NULL
...
// Deletes the worker with the lowest pay
WorkerList* deleteWorstWorker(WorkerList* head)
{
	// In case the list is empty
	if (head == NULL)
	...
	// The worker with the lowest pay is the last worker in the list
	WorkerList* temp_curr = head;
	WorkerList* temp_prev = NULL;
...
// Raises the pay of all employees in the list by a percent
void updateWorker(WorkerList* head, float percent)
{
	// In case the list is empty
	if (head == NULL)
	...
	while (temp_ptr != NULL)
	{
		temp_ptr->data->pay = temp_ptr->data->pay + ((temp_ptr->data->pay * percent) / 100);
		temp_ptr = temp_ptr->next;
	}
}
